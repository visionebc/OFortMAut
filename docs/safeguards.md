...
## §53. A brand never fails — it just says two things in two places

Reported as "the logo is not the right one" on two nodes; it was wrong on
...
7. **Pools match EXACTLY, never by containment.** A pool identifier may be a
   subnet id (`42`) or a name. `192.0.2.0/16` does not cover `192.0.2.0/22` —
