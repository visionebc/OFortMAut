...
**One action can execute: `block_ip`.** Its transport was captured from
fortiweb12 (FortiWeb 7.6.8) on 2026-08-20 by running it — create list, add
member, re-read, delete member, delete list, zero residue. The other three
entries remain specifications and the last gate refuses them, which the console
states as `1/4 action transports have been verified` rather than implying an
armed engine.
...
Then, at execution time and in a different process, two more:

9. **Every gate again.** An action approved at 12:00 executes under 12:03's
   state. In between somebody may have thrown the kill switch, opened a
   maintenance window, or added the source to the trust list — each of which is
   a person deciding *not this*. Trusting the verdict recorded at enqueue time
   carries out a decision the world has since reversed.
10. **Transport preflight** — the appliance itself is asked whether it can
...
the source is inside 192.0.2.0/8" is. Refused proposals are recorded too —
